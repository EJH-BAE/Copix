...
const IGNORE = new Set(['node_modules', '.git', 'out', 'dist', 'dist-electron', '.copix']);
...
app.whenReady().then(() => {
	...
	ipcMain.handle('copix:readFile', async (_e, filePath: string, workspaceRoot?: string) =>
		fs.readFile(resolvePath(filePath, workspaceRoot), 'utf8'));
...
function looksLikeSecretPath(filePath: string): boolean {
	...
	ipcMain.handle('copix:writeFile', async (_e, filePath: string, content: string, workspaceRoot?: string) => {
		if (looksLikeSecretPath(filePath)) {
	...
	ipcMain.handle('copix:runTerminal', async (_e, command: string, workspaceRoot?: string, cwd?: string, elevate?: boolean) => {
			...
			if (isWin && wantsElevate) {
				...
				proc.on('close', async () => {
					try {
						...
						resolve((fileOut || out).trim() || '(elevated command finished)');
					} catch {
