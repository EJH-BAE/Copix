		...
		<article className={`msg-block assistant${live ? ' live' : ''}`}>
			<div className="msg-row assistant">
				<div className="msg-stack">
					{activities && activities.length > 0 && <ChatActivityList activities={activities} />}
					{content && (
...
export function ChatCenter({
	...
	const send = async (text: string) => {
		...
		if (!msg || running || !workspace) return;
	...
	const showLiveTurn = running && (activities.length > 0 || (streaming && !activities.some(a => a.kind === 'think' && a.phase === 'active')));
	const liveContent = activities.some(a => a.kind === 'think' && a.phase === 'active') ? undefined : (streaming || undefined);
	...
	return (
		...
		<div className="chat-center">
			...
			<div className="chat-stream" ref={listRef} onScroll={() => {
				...
				{messages.map(m => (
					m.role === 'user' ? (
						<UserPromptPill key={m.id} content={m.content} onReuse={reusePrompt} />
					) : (
						<AssistantTurn
							key={m.id}
			...
			<div className="composer">
				...
				<div className={`composer-inner${running ? ' disabled' : ''}`}>
					...
					<button
						...
						disabled={running || !input.trim() || !modelReady}
