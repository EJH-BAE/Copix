...
async function main() {
	...
	for (const dir of dirs) {
		...
		if (/^(.build\/distro\/npm\/)?remote$/.test(dir)) {
			const remoteDir = dir;
			if (process.platform === 'win32' && process.env['VSCODE_SKIP_REMOTE_INSTALL'] !== '0') {
				log(remoteDir, 'Skipping remote dependencies on Windows (desktop-only). Set VSCODE_SKIP_REMOTE_INSTALL=0 to install.');
				continue;
			}
			nativeTasks.push(() => {
				const env: NodeJS.ProcessEnv = { ...process.env };
